...
本スキルは `rules/` と `knowledges/` 配下のファイルを以下の優先順位で探す:

1. **`<cwd>/build-score/rules/*.md` / `<cwd>/build-score/knowledges/*.md`**
   プロジェクトで `build-score init` 済みの場合に存在する、組織・プロジェクト固有ルール。
2. **`~/.claude/skills/build-score/templates/rules/*.md` / `~/.claude/skills/build-score/templates/knowledges/*.md`**
   スキル同梱のデフォルト。グローバル clone 直後から利用可能。

どちらも存在しない場合は本 SKILL.md 内の表をデフォルトとして使う。
以降で「ルールファイル」「knowledges」と記述した場合は、常にこの優先順位で解決する。

## 起動時の処理

1. 「ルールファイル参照の規則」に従い `knowledges/` 内のファイルをすべて読み込む（見つかったものだけ使用）
...
「ルールファイル参照の規則」で解決した `knowledges/existing-tools.md` に記載された社内既存ツールの
組み合わせで解決できないかも確認する。
...
「ルールファイル参照の規則」に従って `rules/solution-scale.md` を読み込み、その優先順位に沿って最も軽量な手段を推奨する。見つからない場合は下表のデフォルトを使う。
...
チェックリストではなく、思考の枠組みでレビューする。4段階で実施:

1. **3A. 前提チャレンジ** — ヒアリング結果の前提を明示化し疑う
2. **3B. Dream State Mapping** — 12ヶ月後の理想との整合性を評価
3. **3C. 反転思考 + リスク評価** — 失敗シナリオと対策を具体化
4. **3D. セカンドオピニオン** — Agent ツールで独立したサブエージェントに批判的レビューを委ねる

各段階の具体的な手順・プロンプト・AskUserQuestion の文言は
**`references/critical-review.md`** を参照して実行する。レビュー完了後、3A-3D の結果を統合提示し、verdict 変更があればユーザーに確認する。
...
ROI計算の後、「ルールファイル参照の規則」に従って `rules/effort-estimate.md` を読み込み、その基準で開発工数を1〜5段階で見積もる。見つからない場合は下表のデフォルトを使う。

以下の4軸を評価し、総合して工数レベルを算出する:

1. **機能の技術的難易度** — CRUD か複雑ロジックか、外部API連携の数
2. **利用ツール・構築ツールの難易度** — チームの習熟度、学習コスト
3. **要件定義の難易度** — 利用人数、ステークホルダー数、部門間調整
4. **運用・インフラの複雑さ** — デプロイ環境、監視、メンテナンス
...
5質問の回答をもとにスコアを算出する。「ルールファイル参照の規則」に従って `rules/priority-score.md` を読み込み、そのルブリックを優先して使う。見つからない場合は下表のデフォルトを使う。
...
`knowledges/` 内のファイル (「ルールファイル参照の規則」に従って解決) は、ヒアリング中の判断に利用する:
